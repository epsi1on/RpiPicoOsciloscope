...
namespace SimpleOsciloscope.UI.Render
{
    ...
    public class ThdRender:IScopeRenderer
    {
        ...
        private void DrawGridsHoriz(BitmapContext context, double fMin, double fMax)
        {
            ...
            //using (var ctx = bmp.GetBitmapContext())
                for (int ii = 0; ii <= count; ii++)
                {
                    ...
                    {
                        BitmapContextExtensions.SetPixel(context, (int)yp, i, r, g, b);
                    }
                    ...
                    BitmapContextExtensions.DrawText(context, formattedText, yp, 10, Colors.White);
                    BitmapContextExtensions.FillText(context, formattedText, yp, 10, Colors.White);
        ...
        private void DrawGridsVert(BitmapContext context, double fMin, double fMax)
        {
            ...
            //using (var ctx = context.GetBitmapContext())
                for (int ii = 0; ii <= count; ii++)
                {
                    var y = delta * ii;
                    ...
                    {
                        BitmapContextExtensions.SetPixel(context, i, (int)yp, r, g, b);
                    }
        ...
        public void DoRender(BitmapContext context, SignalPropertyList properties)
        {
            ...
            //using (var ctx = Bmp2.GetBitmapContext())
            {
                ...
                {//calculate thd
                    ...
                    while (fq < maxFreq)
                    {
                        ...
                        {
                            x0 = x0.Truncate(0, context.Width);
                            y0 = y0.Truncate(0, context.Height);

                            x1 = x1.Truncate(0, context.Width);
                            y1 = y1.Truncate(0, context.Height);
                        }
...

            {
                ...
                context.DrawText( formattedText, 0, context.Height - fontSize, Colors.White);
                context.FillText(formattedText, 0, context.Width - fontSize, Colors.White);
            }
