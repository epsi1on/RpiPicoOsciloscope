 ...
 static partial class BitmapContextExtensions
    {
        ...
        public static void DrawPolylineAa(this BitmapContext context, int[] points, Color color,int thickness)
        {
            var col = ConvertColor(color);
            context.DrawPolylineAa(points, col,thickness);
        }
        ...
        /// <param name="points">The points of the polyline in x and y pairs, therefore the array is interpreted as (x1, y1, x2, y2, ..., xn, yn).</param>
        /// <param name="color">The color for the line.</param>
        public static void DrawPolylineAa(this BitmapContext context, int[] points, int color, int thickness)
        {
            //using (var context = bmp.GetBitmapContext())
            {
                // Use refs for faster access (really important!) speeds up a lot!
